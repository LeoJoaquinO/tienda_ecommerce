...
let hardcodedCoupons: Coupon[] = [
    ...
    { id: 3, code: "EXPIRED", discountType: "percentage", discountValue: 99, expiryDate: new Date('2020-01-01'), isActive: true },
];
...
export async function createCoupon(coupon: Omit<Coupon, 'id' | 'isActive'>): Promise<Coupon> {
    // Hardcoded logic
    ...
    const newCoupon: Coupon = { ...coupon, id: newId, isActive: true };
    hardcodedCoupons.push(newCoupon);
    return Promise.resolve(newCoupon);
    
    // --- Database Logic ---
    /*
    const { code, discountType, discountValue, expiryDate } = coupon;
    try {
        const [result] = await pool.query<any>(
            'INSERT INTO coupons (code, discount_type, discount_value, expiry_date, is_active) VALUES (?, ?, ?, ?, ?)',
            [code.toUpperCase(), discountType, discountValue, expiryDate, true]
        );
