...
// This file contains all database actions. It is marked as a server-only file.
// It should be the ONLY file in the application that imports from './db'.
...
export async function updateProductInDb(id: number, product: Partial<Omit<Product, 'id' | 'salePrice'>>): Promise<Product> {
    const imagesString = product.images?.join(',');
    
    const existingProduct = await getProductByIdFromDb(id);
    if(!existingProduct) throw new Error("Product not found");

    const fieldsToUpdate = { ...existingProduct, ...product };
    ...
    const dbFields = {
        ...
        images: imagesString || existingProduct.images.join(','),
...
export async function createCouponInDb(coupon: Omit<Coupon, 'id'>): Promise<Coupon> {
    ...
    try {
        ...
        const [rows] = await pool.query<RowDataPacket[]>('SELECT * FROM coupons WHERE id = ?', [result.insertId]);
...
export async function updateCouponInDb(id: number, coupon: Partial<Omit<Coupon, 'id'>>): Promise<Coupon> {
    const existingCouponRows = await pool.query<RowDataPacket[]>('SELECT * FROM coupons WHERE id = ?', [id]);
    if (existingCouponRows[0].length === 0) throw new Error(`Coupon with ID ${id} not found.`);
    
    const fieldsToUpdate = { ...rowToCoupon(existingCouponRows[0][0]), ...coupon};
    ...
    try {
        ...
        for (const order of orders) {
            const [itemRows] = await pool.query<RowDataPacket[]>(`
                SELECT oi.quantity, oi.price, p.id as product_id, p.name, p.images, p.short_description, p.category, p.stock, p.featured, p.ai_hint, p.discount_percentage, p.offer_start_date, p.offer_end_date, p.description
                FROM order_items oi
            ...
            order.items = itemRows.map(itemRow => ({
                ...
                product: calculateSalePrice(rowToProduct(itemRow))
            }));
        }
        return orders;
    } catch (error) {
